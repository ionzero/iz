...
iz.localize = function(parent) {    
    ...
    // check if we has create_localized.  This saves us from a potentially costly deep-dive into the 
    // localization tree.
    if (!Object.prototype.hasOwnProperty.call(parent, '___parent_prototype') && typeof parent.create_localized === 'function') {
        ...
        if (Object.prototype.hasOwnProperty.call(parent, '___parent_prototype')) {
            parent_proto = parent['___parent_prototype'];
        } else {
        ...
        var new_proxy = Proxy.create({
                                        ...
                                        getOwnPropertyNames: function() {
                                            var keys = { "___get_only_localized_data": 1, '___parent_prototype': 1 }; // hide the 'what changed' function 
                                     ...
                                     }, parent_proto); // Object.getPrototypeOf(parent)); // .constructor.__proto__);
        
        prv.wm_set(new_proxy, parent);
        ...
        new_proxy['___get_only_localized_data'] = function() {
          ...
          for (i = 0; i < proxy_keys.length; i++) {
              attr = proxy_keys[i];
              if (attr === '___get_only_localized_data' || attr === '___parent_prototype') {
                  continue;
              }
        ...
        // we have to set this because the only other way to find it is to walk the weak-map
        // which can become exponential in length as you increase localization depth.
        new_proxy['___parent_prototype'] = parent_proto;
...
iz.enable_harmony_proxies = function() {
    if (typeof(Proxy) === 'undefined' || typeof(WeakMap) === 'undefined') {
        throw new Error("Localize Failure! iz.localize requires Harmony Proxy and WeakMap capability and it is not available. Did you forget to run with --harmony_proxies").stack;
    }
    prv.global_localization_enabled = true;
    prv.localized_parents = new WeakMap();
    prv.wm_get = function(key) {
    ...
    prv.wm_set = function(key, parent) {
        if (key !== null && (typeof key === 'function' || typeof key === 'object')) {
            prv.localized_parents.set(key, parent);
        }
    };
    prv.primitives = {};
    ...
    Array.prototype.concat = function() {
        for (var i = arguments.length - 1; i >= 0; i--) {
            var parent = prv.wm_get(arguments[i]);
            if (parent !== undefined) {
