...
(start code)
var good_duck = iz.Module('Somewhere.Animals.SuperDuck').new_object({ disposition: 'good' });
(end)
*/
proto.new_object = function (args, object_to_localize) {
	...
	if (self.hasOwnProperty('CONSTRUCT') && typeof(self.CONSTRUCT) === 'function') {
...
proto.CONSTRUCT = function(args, object_to_localize) {
	var me = this;
	var parent_constructor = this.super('CONSTRUCT');
	if (parent_constructor) {
		me = parent_constructor(args, object_to_localize);
	}
	if (args) {
        var attr;
        for (attr in args) {
            ...
            if (typeof me[attr] === 'function') {
                ...
                if (iz.Tetchy()) {
                    throw "IZ.Tetchy: " + me._.classname + " has no attribute named " + attr;
                }
            }
        }
    }
...
iz.Package = function (packagename, inheritance, closure) {
    ...
    if (inheritance['extends']) {
        ...
        //Superclass = new prv.loaded_packages[inheritance['extends']].constructor();
		// I think this is more correct.  We want to inherit from the main base class, not an instance of that class.
...
proto.has = function (name, detail) {
    ...
    if (iz.Tetchy()) {
        ...
        for (attr in detail) {
            ...
            if (!valid) {
                throw "iz.Tetchy: " + this._.classname + " tried to use an invalid attribute detail '" + attr + "' in " + name;
            }
        }
    }
	...
	if (typeof parent.create_localized === 'function') {
		...
		var new_proxy = Proxy.create({
										...
										getOwnPropertyNames: function() {
											...
											//console.log("AURRGH: " + util.inspect(keys_to_return));
										...
										getPropertyNames: function() {
											...
											//console.log("AURRGH: " + util.inspect(all_keys));
