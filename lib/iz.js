...
var loaded_packages = {};
var classname_hierarchy = {};

if (typeof global.__iz === 'object') {
    if (typeof global.__iz.loaded_packages === 'object') {
        loaded_packages = global.__iz.loaded_packages;
    }
    if (typeof global.__iz.classname_hierarchy === 'object') {
        classname_hierarchy = global.__iz.classname_hierarchy;
    }
}

var prv = {
    loaded_packages: loaded_packages,
    classname_hierarchy: classname_hierarchy,
    tetchy: false,
    calc_tag: undefined,
...
iz.Package = function (packagename, inheritance, closure) {
    ...
    // instantiates or inherits from this 'class' the prototype object is never created.
    var base_class;
    ...
    add_to_iz_hierarchy(packagename, prv.loaded_packages[packagename]);

    return prv.loaded_packages[packagename];
};

// problem here is when we have something like Foo.thing and Foo.thing.subthing
// we need to resolve this issue.
function add_to_iz_hierarchy(packagename, pkg) {
    var segments = packagename.split('.');
    var key;
    var current_root = prv.classname_hierarchy;
    for (var i = 0; i < segments.length; i++) {
        key = segments[i];
        if (i != segments.length -1) {
            // we are on an intermediate element
            if (typeof current_root[key] !== 'object') {
                current_root[key] = {};
            }
            current_root = current_root[key];
        } else {
            // we are adding our classname
            current_root[key] = { "." : pkg};
        }
    }
}
...
function get_tree_copy(hierarchy_point) {
    var result, keys;

    if (typeof hierarchy_point == 'object') {
        if (typeof hierarchy_point['.'] !== 'undefined') {
            var base = hierarchy_point['.'];
            result = function() {
        ...
        keys = Object.keys(hierarchy_point);
        for (var j = 0; j < keys.length; j++) {
            if (keys[j] != '.') {
                result[keys[j]] = get_tree_copy(hierarchy_point[keys[j]])
            }
        }
        return result;
    }
...
iz.Expose = function(package_or_prefix) {
    var segments, key;
    var root = prv.classname_hierarchy;
    segments = package_or_prefix.split('.');
    var result;
    var base = {};
...
iz.localize = function(parent) {
    ...
    if (!Object.prototype.hasOwnProperty.call(parent, '___localize_info') && typeof parent.create_localized === 'function') {
        ...
        if (Array.isArray(parent)) {
            localize_info.is_array = true;
            proxy_data.length = parent.length;
        } else {
            localize_info.is_array = false;
        }
        ...
        //     parent_proto = localize_info.parent_proto;
        // } else {
        ...
        var handlers, new_proxy, parent_proto;
        if (typeof Proxy == 'function') {
            handlers = {
                getOwnPropertyDescriptor: function(target, name) {
                    ...
                    if (localize_info.is_array && name == 'length') {
                        // console.log('length!!');
                        new_desc = {};
                        for (var key in desc) {
                ...
                ownKeys: function(target) {
                    ...
                    var all_keys = Object.getOwnPropertyNames(target).concat(Object.getOwnPropertyNames(proxy_data));
                    var keys_to_return = [];
            ...
            //  dP"Yb  88     8888b.      88""Yb 88""Yb  dP"Yb  Yb  dP Yb  dP
            // dP   Yb 88      8I  Yb     88__dP 88__dP dP   Yb  YbdP   YbdP
            // Yb   dP 88  .o  8I  dY     88"""  88"Yb  Yb   dP  dPYb    8P
            //  YbodP  88ood8 8888Y"      88     88  Yb  YbodP  dP  Yb  dP
            //
            ...
            //
            // This is the proxy code for node 0.8 through 4.x
            // node 6 updated to direct proxies.  This is the old
            // harmony_proxies support.
            ...
            new_proxy = Proxy.create({
                                        /* This section is only for node below 6.0.0 */
        ...
        localize_info.is_parent = function(obj) {
            return (obj === parent);
        }

        localize_info.get_only_localized_data = function() {
            var out = {};
            var attr, i;
            ...
            for (i = 0; i < proxy_keys.length; i++) {
                ...
                localize_info = proxy_data[attr]['___localize_info'];
                if (typeof proxy_data[attr] === 'object' && typeof localize_info === 'object' &&
                    (typeof localize_info['is_parent'] && localize_info.is_parent(parent[attr])) &&
                    typeof localize_info['get_only_localized_data'] === 'function') {
                    ...
                    if (Array.isArray(proxy_data[attr]) ) {
                        var diff = {};
                        var objectydiff = localize_info.get_only_localized_data();
                        for (var ind in objectydiff ) {
                        ...
                        out[attr] = localize_info.get_only_localized_data();
                    }
                } else {
        ...
        localize_info.get_parent = function() { return localize_info.real_item };
...
iz.enable_harmony_proxies = function() {
    ...
    prv.primitives.Array_shift = Array.prototype.shift;
    ...
    Array.prototype.shift = function() {
        // shift in native code doesn't behave properly when
        // operating on proxies, fall back to pure JS implementation
        var parent = prv.get_parent(this);
        if (parent === undefined) {
            return prv.primitives.Array_shift.apply(this, arguments);
        } else {
            var arr = this;
            var retval = arr[0];
