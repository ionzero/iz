...
if (typeof module === 'object' && typeof module.parent === 'object' && typeof module.parent.require === 'function') {
    ...
    iz_require = function(path, packagename, callback) {
        ...
        for (i = 0; i < prv.search_paths.length; i++) {
            try {
                module.parent.require(prv.search_paths[i] + path);
                loaded_module = true;
                break;
            } catch(e) {
                error = e;
    ...
    iz_require = function(path, packagename, callback) {
        try {
            ...
            throw new Error('Unable to locate ' + packagename + '.  Make sure your file resolution path is set appropriately: ' + e);
        }
        var moduletoload = iz.Module(packagename);
        if (callback && typeof callback === 'function') {
            callback(moduletoload);
...
iz.localize = function(parent) {
    ...
    if (typeof parent.create_localized === 'function') {
        ...
        var thing = prv.wm_get(real_item);
        while(thing !== undefined) {
            real_item = thing;
            //console.log("Typeof thing: " + typeof thing);
            thing = prv.wm_get(real_item);
        }
        var parent_proto = Object.getPrototypeOf(real_item);
        ...
        prv.localized_parents.set(new_proxy, parent);
