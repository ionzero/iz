    ...
    self['___get_only_localized_data'] = function() {
        ...
        for (attr in this._.attributes) {
            if (private_vars[attr] !== undefined) {
                out[attr] = this[attr]();
                if (typeof out[attr] === 'object' && typeof out[attr]['___get_only_localized_data']) {
        ...
        for (attr in args) {
            ...
            if (typeof this[attr] === 'function') {
                if (!this._.attributes[attr]['ro']) {
                    ...
                    this['_set_attribute_value'](attr, args[attr]);
                }
            }
        }
    }
...
iz.Package = function (packagename, inheritance, closure) {
    ...
    if (inheritance['extends'] !== undefined) {
        ...
        if (typeof inheritance['extends'] == 'object') {
            F = function () {};
            F.prototype = inheritance['extends'];
            self = new F();
            var baseIZObj = IZ.prototype;
            self = iz.mixin(self,baseIZObj);
    		self['_'] = {
    			attributes: {}
            };
            self._['ISA'] = [packagename];
            if (!iz.can(self, 'super')) {
    		...
    		//console.log("SELF IS: " + util.inspect(inheritance['extends']));
    		//self = 
        } else {
    ...
    var CONS = function () {
        ...
        //console.log(packagename + " Instantiated");
        return closure(self);
    };
