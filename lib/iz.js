...
iz.Package = function (packagename, inheritance, closure) {
    ...
    // process inheritance.
    if (inheritance['extends'] !== undefined || inheritance['from_prototype_object'] !== undefined) {

        // this needs testing- Inheriting from a non-iz object.  Should work...
        if (inheritance['from_prototype_object'] !== undefined) {
            if (typeof inheritance['from_prototype_object'] === "object") {
                Superclass = inheritance['from_prototype_object'];
            } else {
                throw new Error("Attempting inherit from non-object in " + packagename);
            }
        }
        
		// We want to inherit from the main base class - we need an object to inherit from. We ask
		// for the package's base_class();
		if (Superclass === undefined && inheritance['extends'] !== undefined) {
...
iz.localize = function(parent) {
	...
	if (typeof parent.create_localized === 'function') {
		...
		var proxy_data = {};
		var white_out = {};
		// we have to use Prxy because we may have had to use node-proxy.
