...
describe('IZ Localization:', function () {
	...
	describe('Standard Objects:', function() {
        ...
        it("able to change a localized array", function() {
            var localized_obj = iz.localize(original);

            localized_obj.apartments.push(65);
            assert.deepEqual(localized_obj.apartments, [ 78, 22, '2D', 65]);
			// console.log("******************** " + JSON.stringify(localized_obj.apartments));
            localized_obj.apartments.shift();
			// console.log("********************=> " + JSON.stringify(localized_obj.apartments));
            assert.deepEqual(localized_obj.apartments, [ 22, '2D', 65]);
        });

        it("able to change a localized array, part II", function() {
            var localized_obj = iz.localize(original);

            localized_obj.apartments[3] = 65;
            assert.deepEqual(localized_obj.apartments, [ 78, 22, '2D', 65]);
            localized_obj.apartments.shift();
            assert.deepEqual(localized_obj.apartments, [ 22, '2D', 65]);

            localized_obj.apartments[7] = 'bob';
            assert.equal(localized_obj.apartments[7], "bob");
            assert.equal(localized_obj.apartments[5], undefined);
            assert.equal(localized_obj.apartments.length, 8);
			var arr = [].concat(localized_obj.apartments);
			localized_obj.apartments.shift();
            assert.equal(localized_obj.apartments[0], arr[1]);
            assert.equal(localized_obj.apartments[5], undefined);
            assert.equal(localized_obj.apartments.length, 7);
        });
